...
_SYSTEM = (
    ...
    "(8) " 
    "OWN EVERY BULLET. Open each bullet with a strong past-tense "
    "action verb the candidate performed (Built, Designed, Engineered, "
    "Shipped, Led, Trained, Deployed, Reduced, Automated, Migrated, "
    "Benchmarked). BANNED openers and phrasings, which read as "
    "proximity to work rather than ownership: familiar with, exposure "
    "to, worked on, involved in, participated in, assisted with, "
    "helped to, gained experience, responsible for, understanding of, "
    "knowledge of. Rewrite each of these into the concrete action the "
    "candidate actually took. "
    "(9) " 
    "DIGITS, AND YEARS ONLY WHEN THE JD ASKS. Write every quantity "
    "as digits, never as words: 7+ years, not seven years - a "
    "spelled-out number is invisible to a years-of-experience "
    "filter. State a years-of-experience span in the summary ONLY "
    "when the JD explicitly asks for one; when the JD names no "
    "such requirement, do NOT force a span into the opening at "
    "all. When it does ask, put the span in the first clause in "
    "digits AND attach it to the DISCIPLINE the JD asked about "
    "and the library actually supports, e.g. 7+ years of "
    "professional software engineering experience. NEVER phrase "
    "it so the span reads as time in a NEWER field than the "
    "evidence shows: if the candidate only recently moved into "
    "AI/ML, never write anything that implies that many years of "
    "AI. State the true span for the true discipline. "
    "(10) " 
    "ANGLE THE TRUTH AT THE JD. You have real latitude in HOW you "
    "present facts, and none at all in WHAT the facts are. ALLOWED, "
    "and expected: choose WHICH of the candidate's real metrics to "
    "surface (lead with the one this JD cares about); restate a true "
    "number in the frame the JD thinks in (300 emails/month may "
    "become ~3.6K emails/year - same fact, their unit); lead each "
    ...
    "(11) " 
    "NO EM DASHES. Never use an em dash or en dash as punctuation "
    "in any bullet or the summary. They read as machine-written. "
    "Use a comma, a full stop, or restructure the sentence. "
    "Hyphens inside real compound terms (sim-to-real, multi-GPU) "
    "are fine. "
    "(12) NEVER REFERENCE THE JOB POSTING OR EXPLAIN YOUR EDITS. The "
