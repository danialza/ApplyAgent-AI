...
def rank_projects(jd_text: str, projects: list[Any]) -> list[int] | None:
    ...
    system = (
        ...
        "advocacy.\n"
        "5. Domain-mismatch is FATAL. A robotics project on a "
        "finance/data/web JD ranks AFTER every plausibly-related "
        "project, even if it shares 'AI' / 'ML' tokens. Recruiters "
        "interpret cross-domain projects as 'didn't read the JD' — "
        "they're worse than no project. Same for finance projects on "
        "a robotics JD, NLP-only projects on a CV-vision JD, etc.\n"
        "6. Generic 'AI' overlap is NOT enough to rescue a "
        "cross-domain project. 'embodied AI' / 'vision-language-action' "
        "DON'T make a robotics project relevant for a financial-data "
        "JD. Look at the JD's actual deliverables (SQL pipelines / "
        "automation workflows / agent tools) and match against the "
        "project's actual deliverables.\n"
        "7. When in doubt about a cross-domain project, rank it LAST. "
        "The caller caps the list at 3-4 entries; LAST means cut off.\n\n"
        "Reply JSON: "
